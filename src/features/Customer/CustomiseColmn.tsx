...
const CustomiseColmn = ({ columns, setColumns }: Props) => {
  ...
  const handleVisibilityChange = (id: string) => {
    setLocalColumns(localColumns.map(col => col.id === id ? { ...col, visible: !col.visible } : col));
  };
...
function CustomiseColmn({}: Props) {
  ...
  return (
    <>
      ...
      <Modal open={isModalOpen} style={{ width: "30%" }} onClose={closeModal} className="">
        <div className="p-5">
          ...
          <div>
            {localColumns.map((col, index) => (
              ...
              >
                ...
                />
                <span className="text-textColor text-sm font-semibold">{col.label}</span>
              </div>
            ))}
          </div>
